...
use log::{warn, info};
use queue_models::probe_response::{DestUnreachKind, TraceResponseType};

use crate::schedule::{model::EvaluateRequest, ProbeResponse};
...
impl ProbeStore {
    pub fn request_all(&mut self, req: &EvaluateRequest) {
        for target_addr in req.targets.iter() {
            if self
                .store
                .insert(*target_addr, Target::new(*target_addr, req.net))
                .is_some()
            {
                warn!(
...
pub struct Target {
    pub addr: Ipv6Addr,
    pub ref_net: Ipv6Net, // could probably compute it from addr also
    pub last_hop: Option<Hop>,
    pub target_own_ttl: Option<u8>,
}

impl Target {
    fn new(addr: Ipv6Addr, ref_net: Ipv6Net) -> Self {
        Self {
            addr,
            ref_net,
            last_hop: None,
...
impl ProbeStore {
    pub fn into_request_groups(self) -> Vec<RequestGroup> {
        let group_by = self
            .store
            .into_values()
            .sorted_by_key(|e| e.ref_net)
            .group_by(|e| e.ref_net);

        let mut results = vec![];
        for (ref_net, entries) in group_by.into_iter() {
            let mut group = Self::make_group(ref_net);
    ...
    fn make_group(net: Ipv6Net) -> RequestGroup {
        RequestGroup::new(net)
    }
}
