...
at10 <- at10 |> mutate(err_rate = zmap_received_err / zmap_sent)
at10_err <- at10 |> filter(err_rate > 0)

at11 <- at11 |> mutate(err_rate = zmap_received_err / zmap_sent)
at11_err <- at11 |> filter(err_rate > 0)
...
err_ratio_at10 <- ggplot(data = at10_err, aes(x = err_rate)) + 
  ...
  #geom_vline(aes(xintercept=median(at10$err_rate)), color="#34b0a7")
...
err_ratio_at11 <- ggplot(data = at11_err, aes(x = err_rate)) + 
  ...
  coord_cartesian(ylim = c(0, 0.4)) + 
  scale_y_continuous(name = "% BGP Roots", labels = scales::percent, breaks = seq(0, 0.6, 0.2)) #+
  #geom_vline(aes(xintercept=median(at11$err_rate)), color="#34b0a7")
...
at10 <- at10 |> mutate(in_prefix_rate = yarrp_in_prefix / yarrp_sent)
at10_inp <- at10 %>%
  filter(in_prefix_rate > 0)

at11 <- at11 |> mutate(in_prefix_rate = yarrp_in_prefix / yarrp_sent)
at11_inp <- at11 %>%
  filter(in_prefix_rate > 0)
...
in_prefix_at10 <- ggplot(data = at10_inp, aes(x = in_prefix_rate)) + 
  ...
  #geom_vline(aes(xintercept=median(at10$in_prefix_rate)), color="#34b0a7")
...
in_prefix_at11 <- ggplot(data = at11_inp, aes(x = in_prefix_rate)) + 
  ...
  coord_cartesian(ylim = c(0, 0.5)) + 
  scale_y_continuous(name = "% BGP Roots", labels = scales::percent, breaks = seq(0, 1, 0.2)) #+
  #geom_vline(aes(xintercept=median(at11$in_prefix_rate)), color="#34b0a7")
...
no_yarrp_at10 <- ggplot(data = at10_missed, aes(x = yarrp_miss_rate)) + 
  ...
  coord_cartesian(ylim = c(0, 0.4), xlim = c(0, 1)) + 
  scale_y_continuous(name = "% BGP Roots", labels = scales::percent, breaks = seq(0, 1, 0.2)) #+
